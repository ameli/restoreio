...
def plot_convergence(
    ...
    #         V_all_ensembles_inpainted, save)
    # _plot_convergence_2(
    #         missing_indices_in_ocean_inside_hull, U_all_ensembles_inpainted,
...
def _plot_convergence_1(
    ...
    ax.set_title('Convergence of mean of ensembles population')
...
def _plot_convergence_2(
    ...
    ax.plot(x, yv[:N], color='green',
            label=r'North Velocity Data ($\psi=V$)')
    ...
    # Save plot
    if save:
        filename = 'ensembles_convergence'
...
def _plot_convergence_3(
    ...
    U_mean = numpy.zeros((indices.shape[0], ), dtype=float)
    V_mean = numpy.zeros((indices.shape[0], ), dtype=float)
    ...
    for point_id in range(indices.shape[0]):
        ...
        # Mean of ensembles is the same as the central ensemble
        U_mean[point_id] = U_all_ensembles_inpainted[0, index_I, index_J]
        V_mean[point_id] = V_all_ensembles_inpainted[0, index_I, index_J]
    ...
    # U_mean = numpy.ma.mean(U, axis=0)
    # V_mean = numpy.ma.mean(V, axis=0)
    # U_std = numpy.ma.std(U, axis=0)
    # V_std = numpy.ma.std(V, axis=0)
    ...
    for ensemble in range(num_ensembles):
        U_z_score[ensemble, :] = (U[ensemble, :] - U_mean) / U_std
        V_z_score[ensemble, :] = (V[ensemble, :] - V_mean) / V_std
    ...
    # U_cum_mean_std = numpy.zeros((num_ensembles-1, indices.shape[0]), dtype=float)
    # V_cum_mean_std = numpy.zeros((num_ensembles-1, indices.shape[0]), dtype=float)
    # for ensemble in range(1, num_ensembles):
    #     U_cum_mean_std[ensemble-1, :] = numpy.std(U_cum_mean[:ensemble, :], axis=0)
    #     V_cum_mean_std[ensemble-1, :] = numpy.std(V_cum_mean[:ensemble, :], axis=0)
