...
def _generate_valid_vector_ensembles(
        valid_vector,
        valid_vector_error,
        valid_indices,
        num_ensembles,
    ...
    # Correlation
    cor = _compute_correlation_matrix(valid_indices, acf_length_scale_lon,
    ...
    # Uncomment for plot
    # _plot_valid_vector_ensembles_statistics(
...
def generate_image_ensembles(
    ...
    # Plot acf
    # _plot_auto_correlation(acf_lon, acf_lat, acf_length_scale_lon,
    #                        acf_length_scale_lat)
    ...
    valid_vector_ensembles, eigenvalues, eigenvectors = \
        _generate_valid_vector_ensembles(
    ...
    for ensemble_id in range(num_ensembles):
        masked_image_data_ensembles[ensemble_id+1, :, :] = \
                _convert_valid_vector_to_image(
    ...
    # _plot_kl_transform(lon, lat, eigenvalues, eigenvectors, acf_lon, acf_lat,
    #                    valid_indices, masked_image_data.shape)
...
def _plot_valid_vector_ensembles_statistics(
    ...
    ax[0, 1].title('Standard Deviation Difference')
    ax[0, 1].xlabel('Point')
    ax[0, 1].legend()
...
def _plot_kl_transform(
    ...
    def _plot_eigenvalues():
        ...
        ax2 = ax1.twinx()
        # ax2.semilogx(eigenvalues_cum_sum/eigenvalues_cum_sum[-1],
        #              color='blue', label='Normalized Cumulative sum')
        ax2.plot(eigenvalues_cum_sum/eigenvalues_cum_sum[-1], color='blue',
                 label='Normalized Cumulative sum')
    ...
    def _plot_eigenvectors():
        ...
        def _draw_map(axis):
            ...
            # Basemap (set resolution to 'i' for faster rasterization and 'f'
            # for full resolution but very slow.)
            map = Basemap(
                    ax=axis,
                    projection='aeqd',
            ...
            # Note: We disabled this, sicne it draws ocean with low resolution.
            # Instead, we created a blue facecolor for the axes.
            # map.drawlsmask(land_color='Linen', ocean_color='#C7DCEF',
            #                lakes=True, zorder=-2)
        ...
        for axis in axes.flat:
            ...
            eigenvector_image = _convert_valid_vector_to_image(
                    eigenvectors[:, counter], valid_indices, image_shape)
            _plot_on_each_axis(axis, eigenvector_image,
                               'Mode %d' % (counter+1))
            counter += 1
...
def plot_ensembles_stat(
    ...
    def _plot_convergence():
        ...
        log_diff_means_U_fit = numpy.polyval(
                numpy.polyfit(log_x, log_diff_means_U, 1), log_x)
        diff_means_U_fit = numpy.exp(log_diff_means_U_fit)

        log_diff_means_V = numpy.log(diff_means_V)
        log_diff_means_V_fit = numpy.polyval(
                numpy.polyfit(log_x, log_diff_means_V, 1), log_x)
        diff_means_V_fit = numpy.exp(log_diff_means_V_fit)

        # Plot
        fig, ax = plt.subplots()
        ax.loglog(diff_means_U, color='lightsteelblue', zorder=0,
                  label='East velocity data')
        ax.loglog(diff_means_U_fit, color='darkblue', zorder=1,
                  label='Line fit')
        ax.loglog(diff_means_V, color='palegreen', zorder=0,
                  label='North velocity data')
        ax.loglog(diff_means_V_fit, color='green', zorder=1,
                  label='Line fit')
        ax.set_xlim([x[1], x[-1]])
        ax.grid(True)
    ...
    def _plot_convergence2():
        ...
        def _fit_power_law(x, y):
            ...
            y_fit = 10**(log_y_fit)
            print("Polyfit slope: %f" % poly_fit[0])
            return y_fit

        # --------

        ax.plot(x, yu, color='blue',
                label=(r'East velocity data ($\psi = U$)'))
        ax.plot(x, _fit_power_law(x, yu), '--', color='blue',
                label='East velocity data (fit)')
        ax.fill_between(x, yu_dn, yu_up, color='lightskyblue',
                        label=(r'84\% (std) bound'))

        ax.plot(x, yv, color='green', label=r'North velocity data ($\psi=V$)')
        ax.plot(x, _fit_power_law(x, yv), '--', color='green',
                label='North velocity data (fit)')
        ax.fill_between(x, yv_dn, yv_up, color='palegreen',
                        label=r'84\% (std) bound')
        ...
        ax.set_ylabel(r'$\frac{|\bar{\psi}_s(\mathbf{x}) - ' +
                      r'\bar{\psi}_S(\mathbf{x})|}{\sigma(\mathbf{x})}$')
        plt.title('Convergence of mean of ensembles population')
    ...
    def _plot_scalar_fields(
        ...
        map_1.drawmapscale(min_lon + (max_lon - min_lon) * 0.88, min_lat,
                           mid_lon, mid_lat, Distance, barstyle='simple',
                           units='km', labelstyle='simple', fontsize='7')
        map_2.drawmapscale(min_lon + (max_lon - min_lon) * 0.88, min_lat,
                           mid_lon, mid_lat, Distance, barstyle='simple',
                           units='km', labelstyle='simple', fontsize='7')
        ...
        def _plot_on_each_axis(
            ...
            if log_norm is True:
                ...
                draw = map.pcolormesh(
                        ...
                        norm=matplotlib.colors.log_norm(vmin=min))
                # draw = map.contourf(
                #         lons_grid_on_map, lats_grid_on_map, scalar_field,
        ...
        fig.suptitle(title)
        _plot_on_each_axis(axes[0], map_1, scalar_field_1,
                           'East velocity Data', colormap, log_norm)
        _plot_on_each_axis(axes[1], map_2, scalar_field_2,
                           'North velocity Data', colormap, log_norm)

    # --------------

    # Original (Uninpainted) Data
    # _plot_scalar_fields(U_one_time, V_one_time, 'Velocities', cm.jet,
    #                     shift_colormap_status=False, log_norm=False)
    # _plot_scalar_fields(error_U_one_time, error_V_one_time,
    #                     'Velocity Errors', cm.Reds,
    #                     shift_colormap_status=False, log_norm=False)
    ...
    # _plot_scalar_fields(central_ensemble_east_vel,
    #                     central_ensemble_north_vel, 'Central ensemble',
    #                     cm.jet, shift_colormap_status=False, log_norm=False)
    ...
    #             V_all_ensembles_inpainted_stats['central_ensemble'][0, :]))
    # _plot_scalar_fields(
    #         mean_diff_east_vel, mean_diff_north_vel,
    #         'Normalized first moment deviation w.r.t central ensemble',
    #         cm.Reds, shift_colormap_status=False, log_norm=True)

    # Mean
    # std_east_vel = U_all_ensembles_inpainted_stats['Mean'][0, :]
    # std_north_vel = V_all_ensembles_inpainted_stats['Mean'][0, :]
    # _plot_scalar_fields(std_east_vel, std_north_vel, 'Mean of ensembles',
    #                     cm.jet, shift_colormap_status=False, log_norm=False)
    ...
    # _plot_scalar_fields(std_east_vel, std_north_vel,
    #                     'Standard deviation of ensembles', cm.Reds,
    #                     shift_colormap_status=False, log_norm=True)
    ...
    # _plot_scalar_fields(rmsd_east_vel, rmsd_north_vel,
    #                     'RMSD of ensembles w.r.t central ensemble',
    #                     cm.Reds, shift_colormap_status=False, log_norm=True)
    ...
    # _plot_scalar_fields(nrmsd_east_vel, nrmsd_north_vel,
    #                     'NRMSD of ensembles w.r.t central ensemble',
    #                     cm.Reds, shift_colormap_status=False, log_norm=True)
    ...
    # _plot_scalar_fields(ex_nmsd_east_vel, ex_nmsd_north_vel,
    #                     'Excess Normalized second moment deviation w.r.t ' +
    #                     'central ensemble', cm.Reds,
    #                     shift_colormap_status=False, log_norm=True)
    ...
    # skewness_north_vel[numpy.ma.where(skewness_north_vel < -Trim)] = -Trim
    # _plot_scalar_fields(skewness_east_vel, skewness_north_vel,
    #                     'Normalizsed third moment deviation (Skewness) ' +
    #                     'w.r.t central ensemble', cm.bwr,
    #                     shift_colormap_status=True, log_norm=False)
    ...
    # _plot_scalar_fields(ex_kurtosis_east_vel, ex_kurtosis_north_vel,
    #                     'Normalzied fourth moment deviation (Excess ' +
    #                     'Kurtosis) w.r.t central ensemble', cm.bwr,
    #                     shift_colormap_status=True, log_norm=False)
    ...
    # _plot_scalar_fields(entropy_east_vel, entropy_north_vel,
    #                     'Entropy of ensembles', cm.RdBu_r,
    #                     shift_colormap_status=True, log_norm=False)
    # _plot_scalar_fields(entropy_east_vel, entropy_north_vel,
    #                     'Entropy of ensembles', cm.coolwarm, True)
    # _plot_scalar_fields(entropy_east_vel, entropy_north_vel,
    #                     'Entropy of ensembles', cm.RdYlGn_r, True)

    # Relative Entropy (KL Divergence) with respect to the normal distribution
    # relative_entripyeast_vel = \
    #         U_all_ensembles_inpainted_stats['RelativeEntropy'][0, :]
    # relative_entripynorth_vel = \
    #         V_all_ensembles_inpainted_stats['RelativeEntropy'][0, :]
    # # Trim = 0.1
    # relative_entripyeast_vel[
    #         numpy.ma.where(relative_entripyeast_vel > Trim)] = Trim
    # relative_entripynorth_vel[
    #         numpy.ma.where(relative_entripynorth_vel > Trim)] = Trim
    # # _plot_scalar_fields(relative_entripyeast_vel,
    # #                     relative_entripynorth_vel,
    # #                     'Kullback-Leibler Divergence of ensembles',
    # #                     cm.Reds, shift_colormap_status=True,
    # #                     log_norm=False)
    # _plot_scalar_fields(relative_entripyeast_vel, relative_entripynorth_vel,
    #                     'Kullback-Leibler Divergence of ensembles',
    #                     cm.YlOrRd, True)
    ...
    def _js_distance_of_two_distributions(filename_1, filename_2):
        ...
        def _symmetric_kl_distance(mean_1, mean_2, sigma_1, sigma_2):
            ...
            skld = 0.5 * (_kl_distance(mean_1, mean_2, sigma_1, sigma_2) +
                          _kl_distance(mean_2, mean_1, sigma_2, sigma_1))
            return skld
            ...
            for i in range(field_mean_1.shape[0]):
                for j in range(field_mean_1.shape[1]):
                    ...
                    if field_mean_1.mask[i, j] == False:
                        ...
                        x = numpy.linspace(
                                numpy.min([mean_1-6*sigma_1,
                                           mean_2-6*sigma_2]),
                                numpy.max([mean_1+6*sigma_1,
                                           mean_2+6*sigma_2]), 10000)
                        norm_1 = scipy.stats.norm.pdf(
                                x, loc=mean_1, scale=sigma_1)
                        norm_2 = scipy.stats.norm.pdf(
                                x, loc=mean_2, scale=sigma_2)
                        norm_12 = 0.5*(norm_1+norm_2)
                        jsd = 0.5 * \
                            (scipy.stats.entropy(norm_1, norm_12, base=2) +
                             scipy.stats.entropy(norm_2, norm_12, base=2))
                        if jsd < 0.0:
                            if jsd > -1e-8:
                                jsd = 0.0
                            else:
                                print('WARNING: Negative JS distance: %f'
                                      % jsd)
        ...
        east_jsd = _js_distance(east_mean_t, east_mean_f, east_sigma_t,
                                east_sigma_f)
        ...
        north_jsd = _js_distance(north_mean_t, north_mean_f, north_sigma_t,
                                 north_sigma_f)
    ...
    title_js_1 = 'Jensen-Shannon divergence between full and truncated KL ' + \
        'expansion'
    _plot_scalar_fields(east_jsd, north_jsd, title_js_1, cm.Reds,
                        shift_colormap_status=False, log_norm=False)

    # Plot ratio of energy for truncation
    east_energy_ratio, north_energy_ratio = _ratio_of_truncation_energy(
            filename_1, filename_2)
    title_js_2 = 'Ratio of truncation error energy over total energy of ' + \
        'KL expansion'
    _plot_scalar_fields(east_energy_ratio, north_energy_ratio, title_js_2,
                        cm.Reds, shift_colormap_status=False, log_norm=False)
